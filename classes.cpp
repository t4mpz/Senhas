...
void Menu :: escrever(){
  ...
  esc.open("arc.text" ,std::ofstream :: out | std::ofstream :: app );
  while (opc == 1){
    std::cout << "email : ";
    std::cin >> escrita;
    ...
    std::cin >> opc;
    std::system("cls || clear"); // crossplatform now
  }
  actions_menu(); // no more self object instance
  esc.close();
